...
from app.utils.knowledge_bases_roles import (
    interactive_sleep,
    KnowledgeBaseRoles
)
...
def main():
    ...
    if region_name not in ["us-east-1", "us-west-2"]:
        raise NotSupportedRegionException("The region for needs to be sit to us-east-1 or us-west-2")

    boto3.setup_default_session(region_name=region_name)
    boto3_session = boto3.session.Session()
    bedrock_agent_client = boto3_session.client("bedrock-agent", region_name=region_name)
    s3_client = boto3.client("s3")
    s3_suffix = f"{region_name}-{boto3.client("s3").get_caller_identity()["Account"]}"
    bucket_name = f"bedrock-kb-{s3_suffix}" if not args.bucket_name else args.bucket_name
    kb_roles = KnowledgeBaseRoles(region_name)
    ...
    bedrock_kb_execution_role = kb_roles.create_bedrock_execution_role(bucket_name=bucket_name)
    bedrock_kb_execution_role_arn = bedrock_kb_execution_role["Role"]["Arn"]
    # create security, network and data access policies within OSS
    encryption_policy, network_policy, access_policy = kb_roles.create_policies_in_oss(
    ...
    oss_client = kb_roles.create_os_client(collection_id)
    ...
    if not args.use_s3:
        upload_directory("../data", bucket_name)
